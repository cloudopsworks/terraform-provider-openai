...
| Resource | Purpose | Import ID | Destroy behavior |
| --- | --- | --- | --- |
| `openai_project` | Manage organization projects. | `proj_123` | Archives the project. |
| `openai_service_account` | Manage project service accounts. | `proj_123/svc_acct_123` | Deletes the service account. |
| `openai_project_api_key` | Issue service-account project API keys. | `proj_123/svc_acct_123/key_123` | Deletes the key. |
| `openai_admin_api_key` | Issue organization admin API keys. | `admin_key_123` | Deletes the key. |
| `openai_organization_group` | Manage organization groups. | `group_123` | Deletes the group. |
| `openai_organization_group_user` | Manage group membership. | `group_123/user_123` | Removes the user from the group. |
| `openai_organization_role` | Manage organization custom roles. | `role_123` | Deletes the custom role. |
| `openai_organization_user_role` | Assign organization roles to users. | `user_123/role_123` | Removes the assignment. |
| `openai_organization_group_role` | Assign organization roles to groups. | `group_123/role_123` | Removes the assignment. |
| `openai_project_role` | Manage project custom roles. | `proj_123/role_123` | Deletes the custom role. |

`openai_project_api_key.value` and `openai_admin_api_key.value` are Sensitive and
...
seconds-based create parameter. Admin API key `scopes` are optional create-only
strings sent as the create request `scopes` array; changing scopes replaces the
key and imported keys should leave scopes unset unless replacement is intended.

Admin API key scoped example:
...
| Data source | Purpose |
| --- | --- |
| `openai_project` / `openai_projects` | Read one or list organization projects. |
| `openai_service_account` / `openai_service_accounts` | Read one or list project service accounts. |
| `openai_admin_api_key` / `openai_admin_api_keys` | Read one or list organization admin API key metadata. Key values are redacted. |
| `openai_organization_user` / `openai_organization_users` | Read one or list organization users. Users are data-source only in this provider. |
| `openai_organization_group` / `openai_organization_groups` | Read one or list organization groups. |
| `openai_organization_group_user` / `openai_organization_group_users` | Read one group member or list group members. |
| `openai_organization_role` / `openai_organization_roles` | Read one or list organization roles. |
| `openai_organization_user_role` / `openai_organization_user_roles` | Read one or list user role assignments. |
| `openai_organization_group_role` / `openai_organization_group_roles` | Read one or list group role assignments. |
| `openai_project_role` / `openai_project_roles` | Read one or list project roles. |

## Common workflows

### Bootstrap a project and issue a service-account API key
...
### Manage organization groups and role assignments
...
1. Create a new `openai_admin_api_key` with a distinct name.
2. Store the new `value` in your external secret manager or CI secret store.
3. Update provider initialization to read the new bootstrap key.
4. Run `terraform plan` with the new key.
5. Remove the old key resource or import it and destroy it deliberately.

For critical keys, add `prevent_destroy` and remove that lifecycle protection only
